...
function validateScreenFiles(): boolean {
  ...
  if (allFiles.length === 0) {
    alert('ファイルをアップロードしてください。', 'WARNING')
    return false
  }
  ...
  if (oversizedFile) {
     alert('一つファイルサイズの上限は 20MB です。', 'WARNING')
    return false
  }
  ...
  if (totalSize > MAX_TOTAL_SIZE) {
     alert('１回処理の最大ファイルサイズは 100MB です。', 'WARNING')
    return false
  }
...
const generateScreenWpCode= async () => {
  const confirmed = await confirm(MESSAGES.CONFIRMATION.GENERATE_CODE, MESSAGES.CONFIRMATION.GENERATE_CODE_CONFIRM)
  if (!confirmed) {
    return
  }
 ...
 * 画面仕様書をクリアする
 */
const clearScreenFiles = async()  => {
  const confirmed = await confirm(MESSAGES.CONFIRMATION.CLEAR, MESSAGES.CONFIRMATION.CLEAR_CONFIRM)
  if (!confirmed) {
    return
  }
