...
public class CsvLayoutParseExcel {
	...
	public List<CsvLayout> parseSpecSheet(FileSource source, String sheetName) throws Exception {
		try (InputStream inputStream = source.getInputStream()) {
			List<CsvLayout> result = new ArrayList<>();

			AnalysisEventListener<CsvLayout> listener = new AnalysisEventListener<CsvLayout>() {
				@Override
				public void invoke(CsvLayout row, AnalysisContext context) {
			...
			EasyExcel.read(inputStream, CsvLayout.class, listener).sheet(sheetName).headRowNumber(CommonConstant.START_POS_INDEX) 
					.doRead();
