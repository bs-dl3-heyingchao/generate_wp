...
public interface ShardedCache extends AutoCloseable {
     ...
     * @param cacheTag 自定义标识字符串（如 MD5）
     */
    default <T> void put(String key, T value, long ttl, TimeUnit timeUnit, String cacheTag) {
    ...
    /**
     * 放入缓存，使用自定义 TTL，并指定存储模式和自定义标识字符串。
     *
     * @param key      键
     * @param value    值
     * @param ttl      有效期时长
     ...
     * @param cacheTag 自定义标识字符串（如 MD5）
     */
    default <T> void put(String key, T value, long ttl, TimeUnit timeUnit, CacheStoreMode mode, String cacheTag) {
        put(key, value, ttl, timeUnit, mode);
    }
