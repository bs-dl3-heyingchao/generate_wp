 ...
 * 
 * このクラスは、複数のモジュール（例：Aプロジェクト、Bプロジェクト）から同じ名前の message.properties
 * ファイルが存在する場合でも、すべてを読み込んで1つのプロパティセットに統合します。 これにより、複数のモジュールで共通のメッセージ定義を一元管理でき、
 * 重複や矛盾を防ぐことができます。
 * 
...
public class MessageService {
	...
	static {
		try {
			...
			Enumeration<URL> resources = MessageService.class.getClassLoader().getResources("message.properties");
			...
			if (properties.isEmpty()) {
				throw new RuntimeException(
						"message.properties ファイルが見つかりませんでした。" + "アプリケーションの classpath に message.properties を配置してください。");
			}
		} catch (IOException e) {
	...
	/**
	 * 指定されたキーに対応するメッセージ文字列を返します。 キーが見つからない場合は、"MISSING_MESSAGE_KEY: [キー名]"
	 * の形式でデフォルト値を返します。
	 * 
	...
	/**
	 * 指定されたキーに対応するメッセージ文字列を取得し、引数で指定された値を用いて メッセージをフォーマット（置換）して返します。
	 * 
